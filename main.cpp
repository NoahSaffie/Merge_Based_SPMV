...
inline void MergePath(vector<int> csrRowPtrA, vector<int> csrColIndex, vector<int> csrValueA, vector<double> x, vector<double> &y, double* totalTime[]);
  ...
  for(int i = 0; i < num_of_threads; i++)
    {
      ...
      double results[4];
      __m256d allZeros = _mm256_set1_pd(0.0);
...
inline void RunTests(string filename)
{
  ...
  int isInteger = 0, isReal = 0, isBinary = 0;
  if (f == NULL)
    return;
  ...
  if(isReal)
      ...
      for(int i = 0; i < TESTS_FOR_SINGLE_FILE; i++)
	{
	  ...
	  timeTotal[2]+=singleFuncTime;
	  timeTotal[1]+=singleMergeTime;
	  timeTotal[0]+=singleDiagonalTime;
	}
    }
...
{  
  ...
  #pragma omp parallel for default(none) firstprivate(f, nnzA_mtx) shared(temp_csrRowPtrA, temp_csrRowIndexA, temp_csrColIndexA, temp_csrValueA)
...
{  
  ...
  vector<int> temp_csrRowIndexA(nnzA_mtx); 
  vector<int> temp_csrColIndexA(nnzA_mtx);
  vector<int> temp_csrValueA(nnzA_mtx);

  #pragma omp parallel for default(none) firstprivate(f, nnzA_mtx, isBinary) shared(temp_csrRowPtrA, temp_csrRowIndexA, temp_csrColIndexA, temp_csrValueA)
  for (int k = 0; k < nnzA_mtx; k++)
    {
      int i =-1, j = -1;
      int int_value;
